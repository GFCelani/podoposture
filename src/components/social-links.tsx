...
/**
 * Os quatro destinos sociais. O endereco de cada um vem do painel, ja conferido
 * (https e dominio da propria rede, em conteudo-tipos.ts); o icone fica preso a
 * rede aqui. Adicionar rede pede desenho novo, por isso a lista e' fechada.
 */
const ICONES_DAS_REDES = [
  {
    rede: "facebook",
    label: "Facebook",
...
] as const satisfies readonly { rede: keyof ConteudoContato["redes"]; label: string; path: string }[];
...
export function SocialLinks({
  ...
  return (
    <ul className={`flex items-center gap-4${className ? ` ${className}` : ""}`}>
      {ICONES_DAS_REDES.map((social) => (
        <li key={social.rede}>
          <a
            href={redes[social.rede]}
            target="_blank"
